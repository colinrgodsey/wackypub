...
Implemented in `pkg/agent/hooks.go` (`DiscoverHooks`, `RunHookChain`, `RunUserMessageHooks`), `pkg/agent/macro.go` (`RenderAgentSystemPrompt` `[hook env]` rendering), `pkg/agent/agent_folder.go` (`LoadFolderAgentWithHookEnv`), `pkg/agent/sdk.go` (`AddUserTurn`, `AddAndGenerateTurnStream`, `AddAndGenerateTurn`), and scaffold example `hooks/on-user-message/00-date`. Parent commits `1c99b08` (impl), `41de251` (out-of-band warnings fix); nested repo `tools/wackydiscord` commit `45b4594` (log hook warnings).
...
**Fix.** A `hooks/` directory in the agent workspace, POSIX run-parts structure.
- **Layout & event:** `hooks/<event>/NN-<name>` — e.g. `hooks/on-user-message/00-date`, `01-pre...`. **v1 has exactly one event: `on-user-message`**, firing on every incoming user turn (human messages from CLI/Discord AND A2A peer messages — both are user turns). All executable scripts under the event directory run in ascending numeric order as a CHAIN: each hook sees the env as mutated by its predecessors and the current text; its mutations pass to the next.
- **Hook I/O contract (Colin, 2026-09-03).** A hook is a transformer on two things: env vars and the message text.
  - *Input:* the current environment is passed as the script's process environment (env is shared mutable state); the incoming message text is passed on stdin.
  ...
  - After all hooks run, the final env and text are what the turn build uses.
- **Where effects land.** The final text is the user message the model receives. Hook-set env variables must be model-visible: render ONLY the variables that hooks mutated this turn as a small `[hook env]` section (name=value lines) inside `RenderAgentSystemPrompt` (pkg/agent/macro.go:26-53, called once per turn build immediately before `BuildADKAgentWithConfigAndTracker`) — NEVER dump the full process env into the prompt (host env can contain API keys; leak risk). The hook run happens before render so the deltas exist. Env mutations are TURN-SCOPED (ephemeral).
- **Failure policy.** Non-zero exit or timeout (5s default, configurable) => proceed with the pre-hook env/text unchanged, warn loudly (stderr for CLI; through the result surface otherwise). Hooks must never wedge a turn. Opt-out per call: `WACKYPUB_HOOKS=0`.
- **Trust.** Agent-writable subject to the files-rw ACL — same trust domain as tool files; per Colin, an agent with bash can already do anything, so the hook channel adds no privilege beyond existing tool access. No hooks ship by default; a scaffold example `hooks/on-user-message/00-date` (env-templated) shows the contract.

**Rejected.** Built-in RAG subsystems (a hook shelling to whatever retrieval exists covers it with zero framework); prompt side-channel editing outside the sanctioned hook surface; the earlier pre-receive/pre-send split design (superseded by the single `on-user-message` event per Colin); a workspace-root hook fallback merged with per-agent hooks (per-agent only, v1); a send-side event in v1 (deferred).

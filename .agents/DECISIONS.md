...
## D80: `run_command` preserves captured output when a tool fails, and never macro-expands captured output

Implemented in `pkg/agent/scratchpad.go` (`createScratchpadRaw`, a non-expanding internal create path beside `CreateScratchpad`, used only by the stdout and stderr capture sites) and `pkg/agent/agent_folder.go` (`buildOutputBlocks` extracted from the success path so there is one implementation of the block format, called unconditionally by `executeTool` before it branches on error, with `err.Error()` as the headline and the stderr promotion removed).

**A failing tool's stdout is destroyed.** `executeTool` captures stdout and stderr into buffers and, on non-zero exit, returns an empty string alongside the error, discarding everything the command printed before it failed. stderr survives only incidentally, and only when non-empty, because the error path substitutes `err.Error()` when that buffer is empty. A command that fails quietly yields nothing but `tool bash failed: exit status 1`. The wrapper at 339 also discards, via `if err != nil { return RunCommandResult{}, err }`, though changing that closure is cosmetic for agents and must not be read as addressing this defect. Because `functiontool` drops any value returned alongside a non-nil error, nothing the closure returns in the failure case can reach a model, so the closure edit serves only in-process Go callers and buys nothing on its own. It is listed here because a reader auditing for lost output will find it, and because it looks exactly like the obvious place to fix things, which is how an implementation ended up with a test asserting on a struct the test had itself constructed moments earlier. The timeout path discards output identically, which is the worst instance, since a hung command's partial stdout is usually the whole diagnosis. Partial output is frequently the only record of how far a multi-step command got and therefore of how much of its work is already committed, so the loss makes the obvious recovery, re-running, potentially non-idempotent.

**Captured output is macro-expanded, as an unintended consequence of D28.** The text capture path calls `CreateScratchpad`, and D28 deliberately made that expand scratchpad macros in its payload. That policy is right for agent-authored payloads, where composing entries by reference is the point, and wrong for machine-captured bytes, where it means any tool whose output merely mentions macro syntax fails the whole call. Because the expansion happens after `cmd.Run()` returns, the command has already run, its side effects are committed, and its output is gone. The subtler variant is worse: output referencing an id that exists has that entry's contents silently substituted into text the command never printed, and the call still reports success. This is a data-integrity fault on the success path, reproducible on demand.

**Preserve output on failure through the error message, which is the one channel that survives.** `go.mod` pins `google.golang.org/adk/v2 v2.0.0`, whose `functiontool` does `output, err := f.handler(ctx, input)` then `if err != nil { return nil, err }` (`functiontool/function.go:227-230`), so any payload returned alongside a non-nil error is discarded upstream of this repo and returning both is not available without patching the dependency. In the non-zero-exit and timeout paths, route the captured streams through the existing blocks logic: inline at or under `ScratchpadOutputThreshold`, otherwise stored via the non-expanding create path and referenced from the error using the same block format the success path already emits, so the convention agents already pattern-match on is reused rather than a new one invented. The headline reason becomes always `err.Error()`, dropping the current promotion of stderr content into the headline, which special-cases stderr for no principled reason, since stdout gets no equivalent, and would double-report once stderr also appears as a preserved block. Downstream is traced rather than assumed: `internal/llminternal/base_flow.go:1275-1277` turns the error into `map[string]any{"error": err.Error()}`, that map becomes `genai.FunctionResponse.Response` unmodified, and the OpenAI dialect adapter marshals it with plain `json.Marshal` (`genai/common/payload.go:32`) with no length cap, so nothing between `fmt.Errorf` and the wire alters the string.

**Never expand macros in captured output.** The two text-capture sites for stdout and stderr use a create path that stores bytes verbatim. `ExpandScratchpadMacros` stays exactly where D18 and D28 put it, on `args`, `stdin`, and the agent-facing `create_scratchpad` tool and CLI, where authors mean it. The D48 binary routes are already correct and must not be touched: `CreateBinaryScratchpad` writes bytes verbatim and never expands, which is precisely why it exists.

**Test surface.** `tools_test.go` already has `TestExecuteTool_Failure` and `TestExecuteTool_Timeout` and both are blind to the first defect by accident rather than by design: the failure script writes only to stderr before exiting, and the timeout script writes to stdout after the sleep that gets killed, so neither ever produces stdout before dying. Extending those two with one line each is the cheapest available regression test; new parallel tests are not needed. Two new cases cover the second defect, a successful command whose large output references a dead id and must not fail the call, and one referencing a live id and must not be rewritten. Verified live: both existing tests extended to write to stdout before dying and assert the output survives, and the two new capture-path cases confirm a large output referencing a dead id no longer aborts a completed command while a live id is no longer spliced into captured text.

**Why**: an interface that discards the evidence of a failure teaches agents to suppress failures, and in the session this came out of they did exactly that, appending `|| true` and trailing `echo DONE` to scripts purely to keep their output, which masks the failures exit codes exist to signal. Machine-captured output is not a template and should never be interpreted as one.

**Residual gaps D80 does not close.** A model still cannot structurally distinguish a tool-execution failure from a harness-level one, since an exited command, an unknown command, malformed arguments, and a macro-expansion error all collapse into the same `{"error": "<string>"}` shape. And the CLI `scratchpad create` path is D28's intended expanding surface, so it stays exposed to the false-positive case where legitimate content merely quotes macro syntax, which is documentation and decision records, including this one. Both are worth addressing deliberately later.

**Rejected:** returning partial output with a `nil` error and an exit-status marker in the payload, which makes every failure look like success to anything downstream that trusts the error channel; inlining captured output into the error string unconditionally, which defeats the thresholds auto-capture exists to enforce; documenting the defang workaround or telling agents to avoid scripts that exit non-zero, which treats symptoms when ordinary shell ends in a non-zero exit constantly; and removing macro expansion from `CreateScratchpad` outright, since D28's out-of-band composition is load-bearing and the defect is the shared code path, not the feature.

## D81: Scratchpad entry ids are validated before they are ever used as a path

Implemented in `pkg/agent/scratchpad.go` (`validateScratchpadID`, called from exactly one place, `listScratchpadMatches`, which `findScratchpadFile` and `DeleteScratchpad` now share instead of each globbing on their own).

`findScratchpadFile` resolves an entry with `filepath.Join(spDir, id+"-*")`. `filepath.Join` calls `Clean`, which resolves `..` lexically rather than rejecting it, so an id containing parent-directory segments escapes the calling agent's own `scratchpad/` directory and resolves names in a sibling agent's. The id arrives from four unvalidated sources: `get_scratchpad` passes `args.ID` straight through from a model tool call, `DeleteScratchpad` and `SearchScratchpad` take the same parameter, and macro expansion resolves ids out of text that may have originated in a web fetch, a repository file, or an email body.

Real ids are always four characters drawn from `[0-9a-z]` by `generateRandomID`, so `.`, `/`, and `*` can never appear in a legitimate value. Any id containing them is either an attack or a caller bug, and never something the resolver should attempt. The validator formalizes a guarantee that `docs/agents.md` already publishes to callers rather than inventing a new constraint, and the charset has never been anything else in the file history, so nothing legitimate narrows away. Legacy entry filenames in the shape `id-createdBy.txt`, which `parseFilenameParts` still handles, carry conforming ids too, the drift there was always the line-count segment.

**Validate in one helper, applied at both sites that build a path.** A helper accepting only `^[0-9a-z]{4}$` covers `readScratchpadRaw` (line 364), `SearchScratchpad` (line 600), `GetScratchpad`, and macro expansion when it is the first statement of `findScratchpadFile`. It does **not** cover `DeleteScratchpad`, which never calls that resolver: it duplicates the identical join-and-glob inline at lines 333-336 and then removes every match it finds. A guard confined to the resolver would close disclosure and leave destruction untouched, which is the single most important detail in this decision. `DeleteScratchpad` calls the same validator as its first statement. The duplication itself is then removed: `findScratchpadFile` is rewritten to call the same match-listing helper `DeleteScratchpad` uses, keeping its own first `.txt`-or-`.dat` selection on top of that list, while `DeleteScratchpad` keeps removing every match returned. Adding a validator to both sites without unifying the resolution would leave the duplication that made a single guard look sufficient, which is the condition this decision exists to remove.

Rejection returns an error naming the required id shape rather than reusing `scratchpad entry %q not found`, since the realistic caller here is an agent debugging its own tool call and uniformity buys nothing once the lookup itself is refused.

**Why**: an entry id is an opaque generated token and that property was never encoded anywhere, so every caller silently inherited an assumption that nothing checked. Placing the check directly in the functions that convert an id into a filesystem path, of which there are today exactly two, makes the assumption explicit rather than inherited, which is the same failure mode D80 addresses one layer up, where a single store function quietly served both authored templates and captured bytes. Any future glob-on-id site is expected to carry the validator with it, and a third site added without it is the regression most likely to be left behind.

**Out of scope.** This closes traversal and enumeration. It does not close the friction that produced it, where text that merely quotes macro syntax inside a payload over the auto-capture threshold aborts the call, because that failure happens in `ExpandScratchpadMacros` while extracting the id, before any resolver runs, so no id validation here can reach it. The well-formedness and escape-convention work already tracked in `TODOS.md` is still required separately, and D81 should not be read as having resolved it.

**Blast radius.** Read, search, and delete are all reachable, so this is disclosure and destruction rather than disclosure alone. The trailing wildcard in the lookup pattern means an id ending in a wildcard character enumerates a sibling agent's entries without any prior knowledge of a real id. `WACKYPUB_ALLOWED_AGENTS` does not mitigate this, because that gate lives in the CLI and A2A layer while this is plain filesystem resolution underneath it. Write paths are not exposed: entry creation generates its own id and globs only that generated value.

**Tests.** A malformed id is rejected with no filesystem access attempted, an id containing parent-directory segments cannot resolve an entry that exists in a sibling agent's scratchpad directory in a two-agent temporary workspace, a trailing-wildcard id resolves nothing, and an ordinary four-character id still resolves exactly as before. Both the read path and the delete path need their own case, because they are two independent lookups. Verified live by disabling the validator and watching the suite fail: the read case returned a sibling agent's entry contents through a traversal-and-wildcard id and the delete case destroyed that entry, then passing again once restored, with the source file checksummed against a pre-experiment copy rather than reverted through git. Two existing tests are directly involved. `scratchpad_test.go:56` asserts the literal error text for `GetScratchpad` with the id `missing`, seven characters, which fails the new shape check and must be updated in the same change rather than treated as a regression at review time. `media_test.go:250` hand-writes a `.txt` and a `.dat` entry sharing one id and asserts delete removes both, so it is the tripwire that catches a shared helper being simplified to return one match instead of all. All of this is unit-testable with no binary and no model.
...
**Fix.** A `hooks/` directory in the agent workspace. No hooks ship by default; examples live in the repo (a scaffold example `00-date`). Agent-writable subject to the files-rw ACL — the same trust domain as the tool files; per Colin, an agent with bash can already do anything, so the hook channel adds no privilege beyond existing tool access.
- **Events, v1:** `pre-receive` (an incoming user turn is being prepared for the model) and `pre-send` (an outbound A2A message is about to be sent). More events can be defined later. All hooks share one contract.
- **Ordering:** scripts are named `NN-<event>` (e.g. `00-pre-send`, `01-pre-send`); all scripts for an event run in ascending numeric order as a chain — each hook sees the env as mutated by its predecessors and the current text.
- **Hook I/O contract (Colin, 2026-09-03).** A hook is a transformer on two things: the env vars and the turn/message text.
  - *Input:* the current environment variables are passed as the script's process environment (env is the shared mutable state); the text is passed on stdin — for `pre-receive`, the incoming user message text; for `pre-send`, the outgoing A2A message as JSON (with its `text`).
  - *Output:* JSON on stdout with up to two keys:
    - `env`: object keyed by env var name. Key with value `null` **removes** the variable; a **missing key is ignored** (no change); any other value **replaces** the variable.
    - `text`: nullable. When present (non-null), the text is considered **altered** to that value; `null` or absent means unchanged.
  - Mutations apply for the rest of the turn: later hooks in the chain see updated env, and the final env/text is what the turn build / send uses. For `pre-send`, message identity fields (`to`, `id`, trace metadata) stay immutable; only the body/headers (message text) and env are alterable, and the amended message is schema-validated before send. Malformed/out-of-schema output always means proceed-with-original + warn, never fail-closed.
- **Driver semantics on the new contract.** Date injection is an env mutation (`TODAY=...`): the prompt build renders current env vars into the system prompt inside `RenderAgentSystemPrompt` (macro.go:26-53), which runs once per turn immediately before `BuildADKAgentWithConfigAndTracker` — so hook-set vars are model-visible. RAG is a text alteration: a `pre-receive` hook appends retrieved context to the incoming text.
- **Failure policy:** non-zero exit or timeout (default 5s) => proceed with the pre-mutation env/text, warn loudly. Opt-out per call: `WACKYPUB_HOOKS=0`.

**Rejected.** Built-in RAG subsystems (a pre-receive hook shelling to whatever retrieval exists covers it with zero framework); prompt side-channel editing outside the sanctioned hook surface; a workspace-root hook fallback merged with per-agent hooks (per-agent only, v1); appending hook stdout verbatim to the system prompt (the earlier draft design — unstructured and with no env control; superseded by this JSON env/text contract).

**Open questions (non-blocking).** Whether env mutations persist beyond the current turn (default: turn-only, ephemeral) or across turns; whether non-message A2A calls (cross-agent scratchpad deposits) need a future event (out of scope for `pre-send`).
...
Implemented in `pkg/agent/scratchpad.go` (`scratchpadMacroRegex` leading backslash capture; `ExpandScratchpadMacros` returning warnings and passing through nonexistent IDs literally; `CreateScratchpad` attaching warnings to `ScratchpadEntry`), `pkg/agent/agent_folder.go` (`executeTool` returning warnings separately `(string, []string, error)`, `run_command`/`run_skill_script` populating `RunCommandResult.Warning`, `create_scratchpad` tool returning `CreateScratchpadResult.Warning`), and `cmd/agent.go` (`scratchpadCreateCmd` printing warnings to stderr). Commit pending.

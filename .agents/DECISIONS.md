...
Not yet implemented. Touches `pkg/agent/agent_folder.go` (`RenderAgentSystemPrompt` insertion point, turn request build), `pkg/agent/sdk.go` (user-turn ingestion — both human and A2A user messages route here), and the agent scaffold (`hooks/`).

...
type HTTPOptions struct {
	Client  *http.Client
	Headers http.Header
	// MaxRetries overrides the openai-go client retry budget. Nil means the
	// library default (2). A pointer to 0 disables retries entirely (fail-fast:
	// a single HTTP attempt, no silent retry storm on a slow backend). Negative
	// values are clamped to 0 because option.WithMaxRetries panics on negatives.
	MaxRetries *int
}
